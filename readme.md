...
### 4.29
    ...
    2. 在连通域分析这一块，有暴力BFS/DFS，也有 两遍扫描法+并查集 ，所以可以再加一个
        状态记录此次操作耗费时间，作为实验数据。
### 5.7
    1. 在得到连通域分析的数据之后，就是最后的缺陷分析步骤
    2. 首先进行目标筛选：面积过滤，边界排除
    3. 按照特征进行分类
    4. 全局质量汇总
    5. 最后ui显示和结果输出
### 5.9
    重大突破！Congradulation!
    由于芯片两面的特性不同，所以对于bottom和top图像的处理手段、流程也不一样。
    1. 对于bottom图像，流程是：标准图和模板图同时：双边滤波、Retinex增强、
    Sobel算子。然后金字塔匹配、均值差分、固定阈值分割。连通域分析、缺陷分析。
    2. 对于top图像，流程是：top：待测图和标准图同时：双边滤波。然后图像金字塔。
    待测图和标准图一起：大津法，连通域分析，然后再图像差分。(感觉有点问题，应该是主线程的标准图没更新）
### 5.10
    当前测试得到的步骤：
    1. 双边滤波（高斯滤波效果不好）+Retinex增强+图像金字塔
    2. 阈值分割（大津法会留下花纹和序号,自适应阈值分割会把划痕消除）（还需要考虑边界）
    3. 连通域分析（去除背景噪声，只保留前景）
    4. 图像差分（得到月牙状匹配误差+明显缺陷）
    5. 连通域分析（去除月牙误差，保留缺陷）
    6. 缺陷分析（框出缺陷位置并进行分析）
    新的思路：标准图和待测图都进行处理后得到只含有前景的二值图，然后根据二者的连通域区别得到缺陷连通域，
    最后分析出缺陷类型。
